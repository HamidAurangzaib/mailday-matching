import { useListMatches, useUpdateMatch } from "@workspace/api-client-react";
...
import { Search, Copy, Check, Mail, MapPin, User, Heart, Pencil, X } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { ChildSheet } from "@/components/child-sheet";

function matchDuration(matchDate: string | null | undefined, status: string): string {
  if (!matchDate) return "—";
...
function MatchSheet({
  ...
  const isActive = match.match_status === "Active";
      ...
      <SheetContent className="w-full sm:w-[520px] sm:max-w-[520px] overflow-y-auto bg-white">
        <SheetHeader className="pb-4">
          <div className="flex items-start gap-3">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isActive ? "bg-green-100" : "bg-muted"}`}>
              <Heart className={`w-6 h-6 ${isActive ? "text-green-600" : "text-muted-foreground"}`} />
            </div>
            <div className="space-y-1 flex-1">
              ...
              <div className="flex items-center gap-2 flex-wrap text-sm text-muted-foreground">
                <Badge
                  variant="outline"
                  className={isActive
                    ? "text-green-700 border-green-300 bg-green-50 text-[11px]"
                ...
                <span className={isActive ? "text-green-600 font-medium" : "text-muted-foreground"}>
                  {matchDuration(match.match_date, match.match_status ?? "")}
                </span>
              </div>
              {!isActive && match.close_reason && (
                <div className="text-xs text-muted-foreground mt-1">
                  Closed: <span className="font-medium">{CLOSE_REASON_LABELS[match.close_reason] ?? match.close_reason}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {editingNotes ? (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button size="sm" className="h-7 text-xs" onClick={saveNotes} disabled={updateMatch.isPending}>
                  <Check className="w-3 h-3 mr-1" />
                  {updateMatch.isPending ? "Saving…" : "Save"}
                </Button>
                <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setEditingNotes(false)} disabled={updateMatch.isPending}>
        ...
        {/* Close match — admin only, active matches only */}
        {isAdmin && isActive && (
...
export default function History() {
  ...
  const { data: matches, isLoading, refetch } = useListMatches({ search: search || undefined });
  ...
  const selectedMatch = (matches?.find((m) => m.id === selectedMatchId) ?? null) as MatchWithChildren | null;
        ...
        <div>
          ...
          {!isLoading && (
            <p className="text-sm text-muted-foreground mt-1">
              All {matches?.length ?? 0} matches — sorted by most recent
            </p>
          )}
        </div>
            ...
            {isLoading ? (
              ...
              sorted?.map((match) => (
                ...
                >
                  ...
                  <TableCell>
                    {match.child_a?.child_first_name} ({match.child_a?.age})
                    <div className="text-xs text-muted-foreground">{match.child_a?.parent.state}</div>
                  </TableCell>
                  <TableCell>
                    {match.child_b?.child_first_name} ({match.child_b?.age})
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(match.shared_interests?.length || 0) > 3 && (
                        <span className="text-xs text-muted-foreground ml-1">+{match.shared_interests!.length - 3}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    <span className={`text-sm font-medium ${match.match_status === "Active" ? "text-green-600" : "text-muted-foreground"}`}>
                      {matchDuration(match.match_date, match.match_status ?? "")}
                    </span>
                  </TableCell>
                  <TableCell>
                    <div className="space-y-1">
                      <Badge
                        variant={match.match_status === "Active" ? "default" : "outline"}
                        className={match.match_status === "Active" ? "bg-green-500 hover:bg-green-600 text-[10px]" : "text-muted-foreground text-[10px]"}
                      >
                        {match.match_status}
                      </Badge>
                      {match.close_reason && (
                        <div className="text-[10px] text-muted-foreground">
                          {CLOSE_REASON_LABELS[match.close_reason] ?? match.close_reason}
                        </div>
                      )}
                    </div>
                  </TableCell>
    ...
    <Dialog open={!!matchId} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleClose} disabled={!reason || updateMatch.isPending} variant="destructive">
            Close Match
          </Button>
        </DialogFooter>
      </DialogContent>
